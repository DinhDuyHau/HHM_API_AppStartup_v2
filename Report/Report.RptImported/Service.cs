...
namespace Report.RptImported
{
    public class Service : IReportService
    {
        ...
        public string controller { get; set; } = "rptImported";
        ...
        public CommonObjectModel Execute(Dictionary<string, object> param)
        {
            ...
            CommonObjectModel raw_model = data_utis.GetDataPaging(this.controller, sql, list_paras, obj_param, 1, connectType);
            return raw_model;
        }
        ...
        public Query InitExport(string controller, Dictionary<string, object> param)
        {
            ...
            return new Query()
            {
                ...
                RptTableIndex = M_TABLE_VIEW_INDEX
            };
        }
        ...
        public List<SqlParameter> init(ParamItem obj_param, out string sql)
        {
            ...
            int maxLength = 12;
             ...
             sql = @"declare @voucherList varchar(512)
                    select @voucherList = 'PD1, PD2, PNA, PNB, PNC, PNG, PNE';	
                    select @tu_ngay as date_from, @den_ngay as date_to
                    exec rs_rptImported  @tu_ngay, @den_ngay, @ma_vt, @ma_kh, @ma_kho, '', '', @ma_imei, '', '',
                    '', @nh_vt1, @nh_vt2, @nh_vt3, @nh_vt4, @nh_vt5, @ma_nganh, '', @voucherList,
                    @so_ct1, @so_ct2, @ma_dvcs, @ma_cuahang, @ma_ca, '', '', '', '', 
                    @maxLength, '1', 'ngay_ct', @loai_du_lieu, 'v', @user_id, @admin, @ma_nh_kho, @loginShop";
