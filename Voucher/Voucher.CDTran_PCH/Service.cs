...
namespace Voucher.CDTran_PCH
{
    public class Service : IVoucherService
    {
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("CDTran_PCH");
...
IF EXISTS(SELECT 1 FROM {0} WHERE stt_rec = @stt_rec) BEGIN
	...
	SELECT @q = 'select a.*, b.ten_kh, c.ten_nh from {1}' + @exp + ' a left join vdmkh_acc b on a.ma_kh = b.ma_kh left join dmtknh c on a.tknh = c.tknh where stt_rec = @stt_rec '
