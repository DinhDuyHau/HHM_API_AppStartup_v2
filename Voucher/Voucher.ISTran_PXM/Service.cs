...
using Voucher.ISTran_PXM.Models;
using Genbyte.Component.Voucher.Model;

namespace Voucher.ISTran_PXM
{
    public class Service : IVoucherService
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PXM";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c587$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m587$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i587$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d587$";
        private const string _DETAIL_PARA = "d587";

        //Chuỗi format phục vụ tạo dữ liệu tại bảng inquiry
        public string Operation { get; } = "ma_kh,ma_dvcs,ma_cuahang,ma_ca;#10$,#20$,#30$, #40$; , , , :ma_kho,ma_vt,ma_imei;#10$,#20$,#30$;d587,d587,d587";
        ...
        public CommonObjectModel Inserting(BaseModel data)
        {
            ...
            if (data.details.Any(x => x.Id == index_value) && vc_item.details.Any(x => x.Id == index_value))
            {
                ...
                {
                    List<PXMDetail>? detail_list = JsonSerializer.Deserialize<List<PXMDetail>>((JsonElement)item_model.Data);
                    ...
                    item_detail.Detail_Type = typeof(PXMDetail).Name;
                }
            }
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (vc_item.details.Any(x => x.Id == index_value) && vc_item.details.Any(x => x.Id == index_value))
            {
                ...
                if (item_detail != null)
                {
                    List<PXMDetail> detail_list = item_detail.Data.Cast<PXMDetail>().ToList();
                    if (detail_list != null && detail_list.Count > 0)
                    {
            ...
            if (vc_item.status == "2")
            {
                query = $"EXEC Genbyte$IMEI$PXM$Update '{stt_rec}'";
                service.ExecuteNonQuery(query);
            }
            model.success = true;
...
END";
            ...
            if (ds != null && ds.Tables.Count >= 2)
            {
                ...
                IList<PXMDetail> pr_detail = ds.Tables[1].ToList<PXMDetail>();
        ...
        public List<ImeiState> GetImeis(CommonObjectModel model)
        {
            ...
            if (vc_item.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    ...
                    {
                        var PRDetail = item as PXMDetail;
        ...
        CommonObjectModel checkImeiInsert(VoucherItem vc_item)
        {
            ...
            if (vc_item.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    foreach (var item in item_detail.Data)
                    {
                        var svDetail = item as PXMDetail;
        ...
        CommonObjectModel checkImeiUpdate(VoucherItem vc_item, BaseModel vc_item_old)
        {
            ...
            if (vc_item_old.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    ...
                    foreach (var item in item_detail.Data as List<PXMDetail>)
                    {
                        var svDetail = item as PXMDetail;
                        if (svDetail != null && !string.IsNullOrEmpty(svDetail.ma_imei))
                        {
