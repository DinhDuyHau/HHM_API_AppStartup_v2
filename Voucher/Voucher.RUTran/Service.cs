...
using Voucher.RUTran.Models;
using Genbyte.Component.Voucher.Model;
using Microsoft.Extensions.Configuration;

namespace Voucher.RUTran
{
    public class Service : IVoucherService
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PR5";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c568$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m568$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i568$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d568$";
        private const string _DETAIL_PARA = "d568";

        //Chuỗi format phục vụ tạo dữ liệu tại bảng inquiry
        public string Operation { get; } = "ma_kh,ma_dvcs,ma_cuahang,ma_ca;#10$,#20$,#30$, #40$; , , , :ma_kho,ma_vt,ma_imei;#10$,#20$,#30$;d568,d568,d568";

        /// <summary>
        /// Chuỗi truy vấn khi load chứng từ
        /// </summary>
        public string LoadingQuery { get; } = "exec MokaOnline$App$Voucher$Loading '@@VOUCHER_CODE', '@@MASTER_TABLE', '@@PRIME_TABLE', 'ngay_ct', 'convert(char(6), {0}, 112)', '000000', 0, 'stt_rec', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,rtrim(ma_ca) as ma_ca,ngay_ct,rtrim(so_ct) as so_ct,rtrim(ma_kh) as ma_kh,rtrim(ma_cuahang) as ma_cuahang,rtrim(Dien_giai) as dien_giai, t_so_luong,t_tien_nt,t_thue_nt,t_tt_nt,rtrim(ma_nt) as ma_nt,rtrim(ma_ct) as ma_ct,rtrim(status) as status,rtrim(user_id0) as user_id0,rtrim(user_id2) as user_id2,datetime0,datetime2', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,rtrim(a.ma_ca) as ma_ca,c.ten_ca, rtrim(a.ma_cuahang) as ma_cuahang, ngay_ct,rtrim(so_ct) as so_ct,rtrim(a.ma_kh) as ma_kh,b.ten_kh,rtrim(a.Dien_giai) as dien_giai, t_so_luong,t_tien_nt,t_thue_nt,t_tt_nt,rtrim(ma_nt) as ma_nt,rtrim(a.ma_ct) as ma_ct,rtrim(a.status) as status,rtrim(a.user_id0) as user_id0,rtrim(a.user_id2) as user_id2,a.datetime0,a.datetime2,x.statusname,y.comment,z.comment2,'''' as Hash', 'a left join dmkh b on a.ma_kh = b.ma_kh left join dmca c on a.ma_ca = c.ma_ca left join dmttct x on a.status = x.status and a.ma_ct = x.ma_ct left join @@SYSDATABASE..userinfo y on a.user_id0 = y.id left join @@SYSDATABASE..userinfo z on a.user_id2 = z.id ', '@@ORDER_BY', @@ADMIN, @@USER_ID, 1, 0, '', '', 'ma_cuahang = ''" + Startup.Shop + "''', '@@SYSID'";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("RUTran");
        ...
        public CommonObjectModel Inserting(BaseModel data)
        {
            ...
            vc_item.t_thue = vc_item.t_thue_nt;
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_PARA))
            {
                ...
                query += "\n\n";
                query += $"insert into {detail_table} (stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,ma_vt,ma_sp,ma_bp,so_lsx,dvt,he_so,ma_kho,ma_vi_tri,ma_lo,ma_vv,tk_vt,so_luong,gia_nt,gia,tien_nt,tien,thue_nt,thue,tt_nt,tt,xstatus,xaction,stt_rec_ct,stt_rec0ct, stt_rec_pn, stt_rec0pn, pn_so, pn_ln,stt_rec_kt, stt_rec0kt, kt_so, kt_ln,stt_rec_hd,stt_rec0hd,hd_so,hd_ln, stt_rec_dh,stt_rec0dh,dh_so,dh_ln,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,ma_thue,thue_suat,ma_imei) select stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,ma_vt,ma_sp,ma_bp,so_lsx,dvt,he_so,ma_kho,ma_vi_tri,ma_lo,ma_vv,tk_vt,so_luong,gia_nt,gia_nt,tien_nt,tien_nt,thue_nt,thue_nt,tt_nt,tt_nt,xstatus,xaction,stt_rec_ct,stt_rec0ct, stt_rec_pn, stt_rec0pn, pn_so, pn_ln,stt_rec_kt, stt_rec0kt, kt_so, kt_ln,stt_rec_hd,stt_rec0hd,hd_so,hd_ln, stt_rec_dh,stt_rec0dh,dh_so,dh_ln,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,ma_thue,thue_suat,ma_imei from @{_DETAIL_PARA}";
            }
            ...
            if (vc_item.details.Any(x => x.Id == index_value) && vc_item.details.Any(x => x.Id == index_value))
            {
                ...
                if (item_detail != null)
                {
                    List<PRDetail> detail_list = item_detail.Data.Cast<PRDetail>().ToList();
                    if (detail_list != null && detail_list.Count > 0)
                    {
        ...
        CommonObjectModel checkImeiUpdate(VoucherItem vc_item, BaseModel vc_item_old)
        {
            ...
            if (vc_item_old.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    ...
                    foreach (var item in item_detail.Data as List<PRDetail>)
                    {
                        var svDetail = item as PRDetail;
                        if (svDetail != null && !string.IsNullOrEmpty(svDetail.ma_imei))
                        {
