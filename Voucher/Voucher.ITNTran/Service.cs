...
namespace Voucher.ITNTran
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PXN";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c566$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m566$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i566$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d566$";
        private const string _DETAIL_PARA = "d566";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("ITNTran");
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            // Nếu trạng thái = 0 thì update dat_hang_yn = 1
            if (vc_item.status == "0")
            {
            ...
            else if (vc_item.status == "2")
            {
                string json = JsonSerializer.Serialize(list_imei);
                //create query insert IMEI
                queryIMEI = $"exec Genbyte$IMEI$PXN$Update '{user_id}', '{vc_item.ma_cuahang}', '{stt_rec}', '{vc_item.ngay_ct?.ToString("yyyy-MM-dd")}', '{json}'";
                service.ExecuteNonQuery(queryIMEI);

                queryIMEI = $"EXEC MokaOnline$Voucher$PXNUpdatePNN '{stt_rec}'";
                service.ExecuteNonQuery(queryIMEI);
            }
            if (vc_item.status != "0")
            {
...
IF EXISTS(SELECT 1 FROM {0} WHERE stt_rec = @stt_rec) BEGIN
	...
	SELECT @q = 'select a.*, b.ma_ncc as hddt_ma_ncc, b.mau_hoa_don as hddt_mau_hd, b.so_seri as hddt_so_seri, b.ngay_ct as hddt_ngay_hd,
b.ngay_ky as hddt_ngay_ky, b.so_hoa_don as hddt_so_hd, b.ma_so_thue as hddt_ma_so_thue, b.ma_bi_mat as hddt_ma_tra_cuu, b.status as hddt_status, c.ten_cuahang, d.ten_cuahang as ten_cuahang_n, e.ten_kho, f.ten_kho as ten_khon 
from {1}' + @exp + ' a LEFT JOIN {3}' + @exp + ' b ON a.stt_rec = b.stt_rec  
left join dmcuahang c on a.ma_cuahang = c.ma_cuahang left join dmcuahang d on a.ma_cuahang_n = d.ma_cuahang left join dmkho e on a.ma_kho = e.ma_kho left join dmkho f on a.ma_khon = f.ma_kho
...
END";
            sql = string.Format(sql, this.MasterTable, this.PrimeTable, this.DetailTable, this.EInvoiceTable);
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@vc_id",
        ...
        public CommonObjectModel Finding(List<Dictionary<string, object>> data)
        {
            ...
            CoreService core_service = new CoreService();
            string sql = "EXEC Genbyte$SalesVoucher$Finding_PXN @ngay_bd, @ngay_kt, @ma_cuahang, @ma_ct, @so_ct_bd, @so_ct_kt, @ma_kh, @ma_kho, @ma_kho2, @ma_vt, @ma_imei, @status, @whereClause, @page_index, @page_size, @admin, @user_id, @ext_filter, @order_fields, @filterShopId, @filterShopId_in, @status2, @SysID";
            List<SqlParameter> paras = new List<SqlParameter>();
