...
using Voucher.CRTran_PCG.Models;

namespace Voucher.CRTran_PCG
{
    public class Service : IVoucherService
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PCG";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c548$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m548$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i548$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d548$";
        private const string _DETAIL_PARA = "d548";
        ...
        public CommonObjectModel GetById(string voucherId)
        {
            ...
            if (ds != null && ds.Tables.Count >= 2)
            {
                ...
                IList<PCGDetail> pr_detail = ds.Tables[1].ToList<PCGDetail>();
