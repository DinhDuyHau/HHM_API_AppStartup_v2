...
namespace Voucher.IPTran
{
    public class Service : IVoucherService
    {
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("IPTran");
...
END";
        ...
        public CommonObjectModel Finding(List<Dictionary<string, object>> data)
        {
            ...
            CoreService core_service = new CoreService();
            string sql = "EXEC Genbyte$SalesVoucher$Finding_PNF @ngay_bd, @ngay_kt, @ma_cuahang, @ma_ct, @so_ct_bd, @so_ct_kt, @ma_kh, @ma_kho, @ma_kho2, @ma_vt, @ma_imei, @status, @whereClause, @page_index, @page_size, @admin, @user_id, @ext_filter, @order_fields, @filterShopId, @filterShopId_out, @status2, @SysID";
            List<SqlParameter> paras = new List<SqlParameter>();
