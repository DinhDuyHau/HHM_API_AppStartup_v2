...
namespace Voucher.IRTran_PNM
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PNM";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c578$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m578$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i578$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d578$";
        private const string _DETAIL_PARA = "d578";

        //Chuỗi format phục vụ tạo dữ liệu tại bảng inquiry
        public string Operation { get; } = "ma_kh,ma_dvcs,ma_cuahang,ma_ca;#10$,#20$,#30$, #40$; , , , :ma_kho,ma_vt,ma_imei;#10$,#20$,#30$;d578,d578,d578";
        ...
        public CommonObjectModel Inserting(BaseModel data)
        {
            ...
            if (data.details.Any(x => x.Id == index_value) && vc_item.details.Any(x => x.Id == index_value))
            {
                ...
                {
                    List<PNMDetail>? detail_list = JsonSerializer.Deserialize<List<PNMDetail>>((JsonElement)item_model.Data);
                    ...
                    item_detail.Detail_Type = typeof(PNMDetail).Name;
                }
            }
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (vc_item.details.Any(x => x.Id == index_value) && vc_item.details.Any(x => x.Id == index_value))
            {
                ...
                if (item_detail != null)
                {
                    List<PNMDetail> detail_list = item_detail.Data.Cast<PNMDetail>().ToList();
                    if (detail_list != null && detail_list.Count > 0)
                    {
            ...
            if (vc_item.status == "2")
            {
                query = $"EXEC Genbyte$IMEI$PNM$Update '{stt_rec}'";
                service.ExecuteNonQuery(query);
            }
            model.success = true;
...
END";
            ...
            if (ds != null && ds.Tables.Count >= 2)
            {
                ...
                IList<PNMDetail> pr_detail = ds.Tables[1].ToList<PNMDetail>();
        ...
        public List<ImeiState> GetImeis(CommonObjectModel model)
        {
            ...
            if (vc_item.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    ...
                    {
                        var PRDetail = item as PNMDetail;
        ...
        CommonObjectModel checkImeiInsert(VoucherItem vc_item)
        {
            ...
            if (vc_item.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    foreach (var item in item_detail.Data)
                    {
                        var svDetail = item as PNMDetail;
        ...
        CommonObjectModel checkImeiUpdate(VoucherItem vc_item, BaseModel vc_item_old)
        {
            ...
            if (vc_item_old.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    ...
                    foreach (var item in item_detail.Data as List<PNMDetail>)
                    {
                        var svDetail = item as PNMDetail;
                        if (svDetail != null && !string.IsNullOrEmpty(svDetail.ma_imei))
                        {
