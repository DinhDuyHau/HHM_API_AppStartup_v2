...
namespace Voucher.SVTran_MHA
{
    public class Service : IVoucherService
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "MHA";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c577$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m577$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i577$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d577$";
        private const string _DETAIL_PARA = "d577";

        // bảng lưu dữ liệu chi tiết của thanh toán
        public string PaidTable { get; } = "d577tt$";
        private const string _PAID_PARA = "d577tt";

        //Chuỗi format phục vụ tạo dữ liệu tại bảng inquiry
        public string Operation { get; } = "ma_kh,ma_dvcs,ma_cuahang,ma_ca;#10$,#20$,#30$, #40$; , , , :ma_kho,ma_vt,ma_imei;#10$,#20$,#30$;d577,d577,d577";

        /// <summary>
        /// Chuỗi truy vấn khi load chứng từ
        /// </summary>
        public string LoadingQuery { get; } = "exec MokaOnline$App$Voucher$Loading '@@VOUCHER_CODE', '@@MASTER_TABLE', '@@PRIME_TABLE', 'ngay_ct', 'convert(char(6), {0}, 112)', '000000', 0, 'stt_rec', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,rtrim(ma_ca) as ma_ca,ngay_ct,rtrim(so_ct) as so_ct,rtrim(ma_kh) as ma_kh,rtrim(ma_cuahang) as ma_cuahang,rtrim(Dien_giai) as Dien_giai,t_Ck_nt,t_thue_nt,t_tt_nt,rtrim(ma_nt) as ma_nt,rtrim(ma_ct) as ma_ct,rtrim(status) as status,rtrim(user_id0) as user_id0,rtrim(user_id2) as user_id2,datetime0,datetime2,fcode1', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,rtrim(a.ma_ca) as ma_ca,c.ten_ca, rtrim(a.ma_cuahang) as ma_cuahang, ngay_ct,rtrim(so_ct) as so_ct,rtrim(a.ma_kh) as ma_kh,b.ten_kh,rtrim(a.Dien_giai) as Dien_giai,t_Ck_nt,t_thue_nt,t_tt_nt,rtrim(ma_nt) as ma_nt,rtrim(a.ma_ct) as ma_ct,rtrim(a.status) as status,rtrim(a.user_id0) as user_id0,rtrim(a.user_id2) as user_id2,a.datetime0,a.datetime2,a.fcode1,x.statusname,y.comment,z.comment2,'''' as Hash', 'a left join dmkh b on a.ma_kh = b.ma_kh left join dmca c on a.ma_ca = c.ma_ca left join dmttct x on a.status = x.status and a.ma_ct = x.ma_ct left join @@SYSDATABASE..userinfo y on a.user_id0 = y.id left join @@SYSDATABASE..userinfo z on a.user_id2 = z.id', '@@ORDER_BY', @@ADMIN, @@USER_ID, 1, 0, '', '', 'ma_cuahang = ''" + Startup.Shop + "''', '@@SYSID'";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("SVTran_MHA");
        ...
        public CommonObjectModel Inserting(BaseModel data)
        {
            ...
            if (data.details.Any(x => x.Id == index_tt_value) && vc_item.details.Any(x => x.Id == index_tt_value))
            {
                ...
                if (item_model != null && item_detail != null)
                {
                    List<SVPaidModel>? detail_list = JsonSerializer.Deserialize<List<SVPaidModel>>((JsonElement)item_model.Data);
                    if (detail_list != null && detail_list.Count > 0)
                    {
                        //cập nhật ngày chứng từ
                        detail_list.ForEach(x => x.ngay_ct = vc_item.ngay_ct);
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            query += "\n\n";
            //query += $"insert into {prime_table} (stt_rec,ma_dvcs,ma_ct,loai_ct,so_lo,ngay_lo,ma_nk,ma_gd,ngay_lct,ngay_ct,so_ct,ma_nt,ty_gia,ong_ba,ma_kh,dien_giai,ma_nx,tk,so_ct0,so_seri0,ngay_ct0,t_so_luong,t_tien_nt,t_tien,t_thue_nt,t_thue,tk_thue_no,so_hd_gtgt,t_tt_nt,t_tt,ma_kh2,ma_tt,t_tien0,t_tien_nt0,t_nk,t_nk_nt,t_ttdb,t_ttdb_nt,t_cp_bh,t_cp_bh_nt,t_cp_vc,t_cp_vc_nt,t_cp_khac,t_cp_khac_nt,t_cp,t_cp_nt,gia_thue_yn,nam,ky,xtag,status,datetime0,datetime2,user_id0,user_id2,contract_id,bcontract_id,fee_id,so_dh,job_id,prd_id,dept_id,mo_nbr,t_tien_ck_nt,t_tien_ck,t_ck_nt,t_ck,fcode1,fcode2,fcode3,fdate1,fdate2,fdate3,fqty1,fqty2,fqty3,fnote1,fnote2,fnote3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,new_imei_yn,ma_nvbh) ";
            //query += $" select @stt_rec,@ma_dvcs,@ma_ct,@loai_ct,@so_lo,@ngay_lo,@ma_nk,@ma_gd,@ngay_lct,@ngay_ct,@so_ct,@ma_nt,@ty_gia,@ong_ba,@ma_kh,@dien_giai,@ma_nx,@tk,@so_ct0,@so_seri0,@ngay_ct0,@t_so_luong,@t_tien_nt,@t_tien,@t_thue_nt,@t_thue,@tk_thue_no,@so_hd_gtgt,@t_tt_nt,@t_tt,@ma_kh2,@ma_tt,@t_tien0,@t_tien_nt0,@t_nk,@t_nk_nt,@t_ttdb,@t_ttdb_nt,@t_cp_bh,@t_cp_bh_nt,@t_cp_vc,@t_cp_vc_nt,@t_cp_khac,@t_cp_khac_nt,@t_cp,@t_cp_nt,@gia_thue_yn,@nam,@ky,@xtag,@status,getdate(),getdate(),{user_id},{user_id},@contract_id,@bcontract_id,@fee_id,@so_dh,@job_id,@prd_id,@dept_id,@mo_nbr,@t_tien_ck_nt,@t_tien_ck,@t_ck_nt,@t_ck,@fcode1,@fcode2,@fcode3,@fdate1,@fdate2,@fdate3,@fqty1,@fqty2,@fqty3,@fnote1,@fnote2,@fnote3,@s1,@ma_ca,@ma_cuahang,@s4,@s5,@s6,@s7,@s8,@s9,@new_imei_yn,@ma_nvbh";
...
SELECT is_success, message FROM @check";
            ...
            if (vc_item.status == "2")
            {
                ...
                //Imei.ImeiState? exists = state_imei.FirstOrDefault(x => x.exists_yn == true && x.xuat_yn == false);

                var notExportedImeis = state_imei.Where(x => x.exists_yn == true && x.xuat_yn == false).Select(x => x.ma_imei.Trim()).ToList();

                if (notExportedImeis.Any())
                {
                    result_model.success = false;
                    result_model.message = $"Mã imei chưa được xuất: {string.Join(", ", notExportedImeis)}";
                    return result_model;
                }
            }
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            //    $" ty_gia = @ty_gia, t_so_luong = @t_so_luong, t_thue = @t_thue_nt, t_thue_nt = @t_thue_nt," +
