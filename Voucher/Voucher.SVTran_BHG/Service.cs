...
namespace Voucher.SVTran_BHG
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "BHG";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c582$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m582$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i582$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d582$";
        private const string _DETAIL_PARA = "d582";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("SVTran_BHG");
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_BH_PARA))
            {
                ...
                detail_bh_table = detail_tt_query?.TableName + (detail_bh_query.Partition_yn ? expression : "");
        ...
        public CommonObjectModel Updating(BaseModel data)
        {
            ...
            //tạm cho phép sửa chứng từ tại ngày quá khứ
            /*
...
IF EXISTS(SELECT 1 FROM {0} WHERE stt_rec = @stt_rec) BEGIN
	...
	SELECT @q = @q + CHAR(13) + 'select b1.*, b0.ten_ttbh, b0.dia_chi, b2.ten_dv from {5}' + @exp + ' b1 left join dmtrungtambh b0 on b1.ma_ttbh = b0.ma_ttbh left join dmdichvu b2 on b1.ma_dv = b2.ma_dv where stt_rec = @stt_rec'
