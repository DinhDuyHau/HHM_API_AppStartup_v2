...
{
    public class Service : ISVTranBHDService
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "BHD";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c594$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m594$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i594$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d594$";
        private const string _DETAIL_PARA = "d594";
        // bảng lưu dữ liệu chi tiết của dịch vụ
        public string ServicesTable { get; } = "d594dv$";
        private const string _SERVICES_PARA = "d594dv";
        // bảng lưu dữ liệu chi tiết của chiết khấu
        public string DiscountTable { get; } = "d594ck$";
        private const string _DISCOUNT_PARA = "d594ck";
        // bảng lưu dữ liệu chi tiết của thanh toán
        public string PaidTable { get; } = "d594tt$";
        private const string _PAID_PARA = "d594tt";
        // bảng lưu dữ liệu chi tiết của bảo hành
        public string WarrantyTable { get; } = "d594bh$";
        private const string _WARRANTY_PARA = "d594bh";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("SVTran_BHD");
                    ...
                    if (item_model != null && item_detail != null)
                    {
                        switch (index_value)
                        {
                            ...
                            case 5:
                                ...
                                break;
                            case 6:
                                List<SVDebtModel>? debt_list = JsonSerializer.Deserialize<List<SVDebtModel>>((JsonElement)item_model.Data);
                                if (debt_list != null && debt_list.Count > 0)
                                {
                                    //cập nhật ngày chứng từ
                                    debt_list.ForEach(x => x.ngay_ct = vc_item.ngay_ct);

                                    item_detail.Data = new List<DetailEntity>();
                                    item_detail.Data.AddRange(debt_list);
                                }
                                item_detail.Detail_Type = typeof(SVDebtModel).Name;
                                break;
                            default:
                                break;
                        }
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            query += "\n\n";
            //query += $"insert into {prime_table} (stt_rec, ma_ct, so_ct, ngay_ct, ngay_lct, ma_gd, loai_ct, ma_kh, ma_nt, ty_gia, t_so_luong, t_tien2, t_tien_nt, t_tien2, t_tien_nt2, t_tt, t_tt_nt, ma_thue, t_thue_nt, t_thue, status, t_con_no, t_ck, tien_dat_coc, t_da_tra, ma_dvcs, ma_cuahang, ma_ca, dien_giai, user_id0, user_id2, datetime0, datetime2, lap_dh_lk, ma_nvbh) ";
            //query += $" select @stt_rec, @ma_ct, @so_ct, @ngay_ct, @ngay_ct, @ma_gd, @loai_ct, @ma_kh, @ma_nt, @ty_gia, @t_so_luong, @t_tien, @t_tien_nt, @t_tien2, @t_tien_nt2, @t_tt, @t_tt_nt, @ma_thue, @t_thue_nt, @t_thue, @status, @t_con_no, @t_ck, @tien_dat_coc, @t_da_tra, @ma_dvcs, @ma_cuahang, @ma_ca, @dien_giai, {user_id}, {user_id}, getdate(), getdate(), 0, @ma_nvbh";
            query += $"{insert_prime_table_query}";

            //insert các bảng chi tiết
            DetailQuery? detail_query = null;
            string detail_table = "", services_table = "", discount_table = "", paid_table = "", warranty_table = "", debt_table = "";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _SERVICES_PARA))
            {
                ...
                //query += "\n\n";
                //query += $"insert into {services_table} (stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, line_nbr, ma_dv, dvt, he_so, so_luong) select stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, line_nbr, ma_dv, dvt, he_so, so_luong from @{_SERVICES_PARA}";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DISCOUNT_PARA))
            {
                ...
                //query += "\n\n";
                //query += $"insert into {discount_table} (stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, line_nbr, ma_ck, ngay_bd, ngay_kt, tl_ck, tien_ck, ma_nvbh_i, ma_vt) select stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, line_nbr, ma_ck, ngay_bd, ngay_kt, tl_ck, tien_ck, ma_nvbh_i, ma_vt from @{_DISCOUNT_PARA}";
            ...
            if (!string.IsNullOrEmpty(debt_table))
                query += $"exec fs_UpdateNullToTable '{debt_table}', '{debt_table}', 'stt_rec = ''{stt_rec}''' \n";
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            //    $" t_tt_nt = @t_tt_nt, t_tt = @t_tt, t_ck = @t_ck, t_con_no = @t_con_no, t_da_tra = @t_da_tra, tien_dat_coc = @tien_dat_coc, ma_gd = @ma_gd, loai_ct = @loai_ct, user_id2 = {user_id}, datetime2 = getdate(), lap_dh_lk = @lap_dh_lk, ma_nvbh = @ma_nvbh ";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_PARA))
            {
                ...
                query += $"insert into {detail_table} (stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,ma_vt,km_yn,ma_sp,ma_bp,so_lsx,dvt,he_so,ma_kho,ma_vi_tri,ma_lo,ma_vv,tk_vt,so_luong,gia_nt,gia,gia_nt2,gia2,tien_nt,tien,thue,thue_nt,tt,tt_nt,xstatus,xaction,tien2,tien_nt2,cp_bh,cp_bh_nt,cp_vc,cp_vc_nt,cp_khac,cp_khac_nt,cp,cp_nt,stt_rec_ct,stt_rec0ct,gia_ban0,gia_ban_nt0,gia_ban,gia_ban_nt,tl_ck,gia_ck,gia_ck_nt,ck,ck_nt,sl_dh,sl_xuat,sl_giao,stt_rec_dh,stt_rec0dh,dh_so,dh_ln,stt_rec_px,stt_rec0px,px_so,px_ln,stt_rec_gh,stt_rec0gh,stt_rec_pn,stt_rec0pn,tk_gv,tk_dt,tk_ck,tk_cpbh,px_gia_dd,ma_nvbh_i,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,ma_thue,tk_thue_no,tk_thue_co,thue_suat,ma_imei,no_km_yn, gia_vat, imei_mua) ";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DISCOUNT_PARA))
            {
                ...
                //query += $"insert into {discount_table} (stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, line_nbr, ma_ck, ngay_bd, ngay_kt, tl_ck, tien_ck, ma_nvbh_i, ma_vt) ";
                //query += $"select stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, line_nbr, ma_ck, ngay_bd, ngay_kt, tl_ck, tien_ck, ma_nvbh_i, ma_vt from @{_DISCOUNT_PARA}";
        ...
        public CommonObjectModel Delete(string voucherId)
        {
            ...
            sql += $"delete from {this.DebtTable + ngay_ct.ToString("yyyyMM")} where stt_rec = @vc_id \n";
            paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@vc_id",
...
END";
        ...
        */
        ...
        public CommonObjectModel UpdateForm(string id)
        {
            CoreService core_service = new CoreService();

            //check sql injection
            if (!core_service.IsSQLInjectionValid(id))
                throw new Exception(ApiReponseMessage.Error_InputData);
            string sql = @"DECLARE @q NVARCHAR(4000), @stt_rec CHAR(13), @exp CHAR(6)
                        SET @stt_rec = @vc_id
                        IF EXISTS(SELECT 1 FROM {0} WHERE stt_rec = @stt_rec) BEGIN
                        	SELECT @exp = CONVERT(CHAR(6), ngay_ct, 112) FROM {0} WHERE stt_rec = @stt_rec
                        	SELECT @q = 'UPDATE {1}' + @exp + ' set lap_dh_lk = 1  where stt_rec = @stt_rec '
                        	EXEC sp_executesql @q, N'@stt_rec CHAR(13)', @stt_rec = @stt_rec
                        END";
            sql = string.Format(sql, this.MasterTable, this.PrimeTable);
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@vc_id",
            ...
            core_service.ExecuteNonQuery(sql, paras);
            CommonObjectModel model = new CommonObjectModel()
            {
                success = true,
                message = "",
