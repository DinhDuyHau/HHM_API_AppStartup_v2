...
namespace Voucher.SVTran_BHK
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "BHK";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c589$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m589$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i589$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d589$";
        private const string _DETAIL_PARA = "d589";


        //Bảng lưu dữ liệu chi tiết của dịch vụ
        public string DetailDVTable { get; } = "d589dv$";
        private const string _DETAIL_DV_PARA = "d589dv";


        //Bảng lưu dữ liệu chi tiết của chiết khấu
        public string DetailCkTable { get; } = "d589ck$";
        private const string _DETAIL_CK_PARA = "d589ck";


        //Bảng lưu dữ liệu chi tiết của thanh toán
        public string DetailTtTable { get; } = "d589tt$";
        private const string _DETAIL_TT_PARA = "d589tt";


        //Bảng lưu dữ liệu chi tiết của bảo hành
        public string DetailbhTable { get; } = "d589bh$";
        private const string _DETAIL_BH_PARA = "d589bh";


        //Bảng lưu dữ liệu chi tiết của hàng thu cũ
        public string DetailHtcTable { get; } = "d589htc$";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("SVTran_BHK");
        ...
        public CommonObjectModel Inserting(BaseModel data)
        {
            ...
            if (data.details.Any(x => x.Id == index_dv_value) && vc_item.details.Any(x => x.Id == index_dv_value))
            {
                ...
                if (item_model != null && item_detail != null)
                {
                    ...
                    if (detail_list != null && detail_list.Count > 0)
                    {
                        ...
                        item_detail.Data = new List<DetailEntity>();
                        item_detail.Data.AddRange(detail_list);
                        serviceModels = detail_list.Cast<ServiceDetailBase>().ToList();
                    }
            ...
            //convert dữ liệu chi tiết chứng từ HTC
            // id = 6 ==> type: HtcDetail
            var index_htc_value = 6;
            if (data.details.Any(x => x.Id == index_htc_value) && vc_item.details.Any(x => x.Id == index_htc_value))
            {
                DetailItemModel? item_model = data.details.FirstOrDefault(x => x.Id == index_htc_value);
                VoucherDetail? item_detail = vc_item.details.FirstOrDefault(x => x.Id == index_htc_value);

                if (item_model != null && item_detail != null)
                {
                    List<HtcDetail>? detail_list = JsonSerializer.Deserialize<List<HtcDetail>>((JsonElement)item_model.Data);
                    if (detail_list != null && detail_list.Count > 0)
                    {
                        //cập nhật ngày chứng từ
                        detail_list.ForEach(x => x.ngay_ct = vc_item.ngay_ct);
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            //insert các bảng chi tiết htc
            DetailQuery? detail_htc_query = null;
            string detail_htc_table = "";
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_HTC_PARA))
            {
                detail_htc_query = voucherQuery.Details.FirstOrDefault(x => x.ParaName == _DETAIL_HTC_PARA);
                detail_htc_table = detail_htc_query?.TableName + (detail_htc_query.Partition_yn ? expression : "");

                query += "\n\n";
                query += detail_htc_query?.QueryString;
                query += "\n";
                query += $"update @{_DETAIL_HTC_PARA} set line_nbr = row_id$, stt_rec0 = right(row_id$ + 1000, 3), stt_rec = @stt_rec, ma_ct = @ma_ct, ngay_ct = @ngay_ct, so_ct = @so_ct, ma_cuahang = @ma_cuahang, ma_ca = @ma_ca where 1=1";
                query += "\n\n";
                query += $"insert into {detail_htc_table} (stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,ma_vt,ma_sp,ma_bp,so_lsx,dvt,he_so,ma_kho,ma_vi_tri,ma_lo,ma_vv,tk_vt,so_luong,gia_nt,gia,gia_nt0,gia0,tien_nt,tien,ma_thue,tk_thue,thue_suat,thue,thue_nt,tt,tt_nt,xstatus,xaction,tien0,tien_nt0,ma_thue_nk,thue_suat_nk,tk_thue_nk,nk,nk_nt,ma_thue_ttdb,thue_suat_ttdb,tk_thue_ttdb,ttdb,ttdb_nt,cp_bh,cp_bh_nt,cp_vc,cp_vc_nt,cp_khac,cp_khac_nt,cp,cp_nt,stt_rec_ct,stt_rec0ct,ct_so,ct_ln,stt_rec_dh,stt_rec0dh,dh_so,dh_ln,stt_rec_pn,stt_rec0pn,pn_so,pn_ln,tien_hang,tien_hang_nt,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,gia_ck_nt,gia_ck,tien_ck_nt,tien_ck,tl_ck,ck_nt,ck,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,ma_imei,ma_loai,new_imei_yn) ";
        ...
        public CommonObjectModel Updating(BaseModel data)
        {
            ...
            */
            ...
            //convert dữ liệu chi tiết chứng từ hàng thu cũ
            // id = 6 ==> type: HtcDetail
            var index_htc_value = 6;
            if (data.details.Any(x => x.Id == index_htc_value) && vc_item.details.Any(x => x.Id == index_htc_value))
            {
...
SELECT is_success, message FROM @check";
            ...
            //        {
            //            checkModel = CommonService.checkDicountCode(paidDetails.Find(x => x.ma_thanhtoan.Trim() == "MAGG"));
            //        }
            //    }
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_TT_PARA))
            {
                ...
                string update_paid_query = VoucherUtils.getPaidQuery(new TTDetail(), detail_tt_table, _DETAIL_TT_PARA, 2, paid_code_not_update);
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_HTC_PARA))
            {
                detail_query = voucherQuery.Details.FirstOrDefault(x => x.ParaName == _DETAIL_HTC_PARA);
                detail_htc_table = detail_query?.TableName + (detail_query.Partition_yn ? expression : "");

                query += "\n\n";
                query += detail_query?.QueryString;
                query += "\n";
                query += $"update @{_DETAIL_HTC_PARA} set line_nbr = row_id$, stt_rec0 = right(row_id$ + 1000, 3), stt_rec = @stt_rec, ma_ct = @ma_ct, ngay_ct = @ngay_ct, so_ct = @so_ct where 1=1";
                query += "\n\n";

                //xóa dữ liệu cũ (bảng detail) và insert dữ liệu mới
                query += $"delete from {detail_htc_table} where stt_rec = @stt_rec \n";
                query += $"insert into {detail_htc_table} (stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,ma_vt,ma_sp,ma_bp,so_lsx,dvt,he_so,ma_kho,ma_vi_tri,ma_lo,ma_vv,tk_vt,so_luong,gia_nt,gia,gia_nt0,gia0,tien_nt,tien,ma_thue,tk_thue,thue_suat,thue,thue_nt,tt,tt_nt,xstatus,xaction,tien0,tien_nt0,ma_thue_nk,thue_suat_nk,tk_thue_nk,nk,nk_nt,ma_thue_ttdb,thue_suat_ttdb,tk_thue_ttdb,ttdb,ttdb_nt,cp_bh,cp_bh_nt,cp_vc,cp_vc_nt,cp_khac,cp_khac_nt,cp,cp_nt,stt_rec_ct,stt_rec0ct,ct_so,ct_ln,stt_rec_dh,stt_rec0dh,dh_so,dh_ln,stt_rec_pn,stt_rec0pn,pn_so,pn_ln,tien_hang,tien_hang_nt,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,gia_ck_nt,gia_ck,tien_ck_nt,tien_ck,tl_ck,ck_nt,ck,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,ma_imei,ma_loai,new_imei_yn) ";
                query += $" select stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,ma_vt,ma_sp,ma_bp,so_lsx,dvt,he_so,ma_kho,ma_vi_tri,ma_lo,ma_vv,tk_vt,so_luong,gia_nt,gia,gia_nt0,gia0,tien_nt,tien,ma_thue,tk_thue,thue_suat,thue,thue_nt,tt,tt_nt,xstatus,xaction,tien0,tien_nt0,ma_thue_nk,thue_suat_nk,tk_thue_nk,nk,nk_nt,ma_thue_ttdb,thue_suat_ttdb,tk_thue_ttdb,ttdb,ttdb_nt,cp_bh,cp_bh_nt,cp_vc,cp_vc_nt,cp_khac,cp_khac_nt,cp,cp_nt,stt_rec_ct,stt_rec0ct,ct_so,ct_ln,stt_rec_dh,stt_rec0dh,dh_so,dh_ln,stt_rec_pn,stt_rec0pn,pn_so,pn_ln,tien_hang,tien_hang_nt,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,gia_ck_nt,gia_ck,tien_ck_nt,tien_ck,tl_ck,ck_nt,ck,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,ma_imei,ma_loai,new_imei_yn from @{_DETAIL_HTC_PARA}";
            }
            else
        ...
        public void updateService(VoucherItem vc_item)
        {
            if (vc_item.status == "2" && vc_item.details.FirstOrDefault(x => x.Name == _DETAIL_DV_PARA) != null)
            {
                ...
                if (list_key != null && list_key.Count > 0)
                {
                    ...
                    flag = CommonService.updateServiceDetailTable(this.DetailDVTable + expression, vc_item.stt_rec, list_key);
                    if (flag)
                    {
                        // UPdate active key
