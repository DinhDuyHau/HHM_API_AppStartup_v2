            ...
            if (vc_item.status == "2")
            {
                ...
                if (!string.IsNullOrEmpty(detail_dv_table) && vc_item.details.FirstOrDefault(x => x.Name == _DETAIL_DV_PARA) != null)
                {
                    ...
                    if (list_key != null && list_key.Count > 0)
                    {
                        ...
                        flag = CommonService.updateServiceDetailTable(this.DetailDVTable + expression, stt_rec, list_key);
                        if (flag)
                        {
                            // UPdate active key
...
END";
            sql = string.Format(sql, this.MasterTable, this.PrimeTable, this.DetailTable, this.DetailDVTable,
                this.DetailCkTable, this.DetailTtTable, this.DetailbhTable, this.DetailHtcTable, this.EInvoiceTable);
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@vc_id",
