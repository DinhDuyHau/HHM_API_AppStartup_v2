...
namespace Voucher.SVTran_BHC
{
    public class Service : IVoucherService
    {
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("SVTran_BHC");
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _ECOMMERCE_PARA))
            {
                ...
                query += "\n\n";
                query += $"insert into {ecommerce_table} (stt_rec, ma_ct, ngay_ct, so_ct, ma_kh_tmdt, ma_dh, doanh_thu, giam_gia_tmdt , giam_gia_hhm, thanh_tien, ma_dvvc, ngay_nhan_hang, phi_vc, nguoi_nhan_hang, ma_van_don, tien_phi_01, tien_phi_02, tien_phi_03, tien_phi_04, tien_phi_05, tien_phi_06, tien_phi_07, tien_phi_08, tien_phi_09, tien_phi_10, phi_hoang_ha)" +
                                                $" select stt_rec, ma_ct, ngay_ct, so_ct, ma_kh_tmdt, ma_dh, doanh_thu, giam_gia_tmdt , giam_gia_hhm, thanh_tien, ma_dvvc, ngay_nhan_hang, phi_vc, nguoi_nhan_hang, ma_van_don, tien_phi_01, tien_phi_02, tien_phi_03, tien_phi_04, tien_phi_05, tien_phi_06, tien_phi_07, tien_phi_08, tien_phi_09, tien_phi_10, phi_hoang_ha from @{_ECOMMERCE_PARA}";
            }
            query += "\n\n";
...
IF @status_older <> '0' AND @status_older <> '3' BEGIN
