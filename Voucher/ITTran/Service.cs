...
namespace Voucher.ITTran
{
    public class Service : IVoucherService
    {
        ...
        public string Operation { get; } = "ma_kh,loai_ct,ma_nt,ma_cuahang,ma_ca;#10$,#15$,#20$,#30$, #40$; , , , , :ma_kho,ma_vt,tk_vt,tk_gv,tk_dt,ma_imei;#10$,#20$,#30$,#40$,#50$,#60$;d585,d585,d585,d585,d585,d585";

        /// <summary>
        /// Chuỗi truy vấn khi load chứng từ
        /// </summary>
        public string LoadingQuery { get; } = "exec MokaOnline$App$Voucher$Loading '@@VOUCHER_CODE', '@@MASTER_TABLE', '@@PRIME_TABLE', 'ngay_ct', 'convert(char(6), {0}, 112)', '000000', 0, 'stt_rec', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,ngay_ct,rtrim(so_ct) as so_ct,rtrim(ma_kh) as ma_kh,rtrim(Dien_giai) as Dien_giai,t_tien_nt2,t_Ck_nt,t_thue_nt,t_tt_nt,rtrim(ma_nt) as ma_nt,rtrim(ma_ct) as ma_ct,rtrim(status) as status,rtrim(user_id0) as user_id0,rtrim(user_id2) as user_id2,datetime0,datetime2', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,ngay_ct,rtrim(so_ct) as so_ct,rtrim(a.ma_kh) as ma_kh,b.ten_kh,rtrim(a.Dien_giai) as Dien_giai,t_tien_nt2,t_Ck_nt,t_thue_nt,t_tt_nt,rtrim(ma_nt) as ma_nt,rtrim(a.ma_ct) as ma_ct,rtrim(a.status) as status,rtrim(a.user_id0) as user_id0,rtrim(a.user_id2) as user_id2,a.datetime0,a.datetime2,x.statusname,y.comment,z.comment2,'''' as Hash', 'a left join dmkh b on a.ma_kh = b.ma_kh left join dmttct x on a.status = x.status and a.ma_ct = x.ma_ct left join @@SYSDATABASE..userinfo y on a.user_id0 = y.id left join @@SYSDATABASE..userinfo z on a.user_id2 = z.id ', '@@ORDER_BY', @@ADMIN, @@USER_ID, 1, 0, ''";
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_PARA))
            {
                ...
                query += "\n\n";
                query += $"insert into {detail_table} (stt_rec, stt_rec0, ma_cuahang, ma_ca, ma_ct, ngay_ct, so_ct, line_nbr, ma_vt, ma_kho, so_luong, gia_nt, gia_nt2, tien_nt, tien_nt2) select stt_rec, stt_rec0, ma_cuahang, ma_ca, ma_ct, ngay_ct, so_ct, line_nbr, ma_vt, ma_kho, so_luong, gia_nt2, gia_nt2, tien_nt2, tien_nt2 from @{_DETAIL_PARA}";
            }
            query += "\n\n";
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            query += $"update {prime_table} set status = @status, ma_ca = @ma_ca, dien_giai = @dien_giai, ma_kh = @ma_kh, ma_nt = @ma_nt," +
                ...
                $" t_tt_nt = @t_tt_nt, t_tt = @t_tt_nt, ma_gd = @ma_gd, loai_ct = @loai_ct, user_id2 = {user_id}, datetime2 = getdate()";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_PARA))
            {
                ...
                query += $"insert into {detail_table} (stt_rec, stt_rec0, ma_cuahang, ma_ca, ma_ct, ngay_ct, so_ct, line_nbr, ma_vt, ma_imei, ma_kho, so_luong, gia2, tien2, gia_nt2, tien_nt2) ";
                query += $"select stt_rec, stt_rec0, ma_cuahang, ma_ca, ma_ct, ngay_ct, so_ct, line_nbr, ma_vt, ma_imei, ma_kho, so_luong, gia_nt2, tien_nt2, gia_nt2, tien_nt2 from @{_DETAIL_PARA}";
            }
            query += "\n\n";
            query += "select @stt_rec as stt_rec";
            ...
            if (vc_item.status == "2")
            {
                ...
                string queryIMEI = "declare @detail IMEITable \n";
                foreach (var item in list_imei)
                {
                    queryIMEI += $"INSERT INTO @detail SELECT '{item.ma_imei}', '{item.ma_vt.Trim()}', '{item.ma_kho.Trim()}', '{item.gia_nt0}' \n";
                }
                queryIMEI += $"exec Genbyte$IMEI$PXB$Update '{user_id}', '{vc_item.ma_cuahang}', '{stt_rec}', '{vc_item.ngay_ct?.ToString("yyyy-MM-dd")}', @detail";
