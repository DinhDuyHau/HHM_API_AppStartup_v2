...
IF EXISTS(SELECT 1 FROM {0} WHERE stt_rec = @stt_rec) BEGIN
	...
	SELECT @q = 'select a.*, b.image from {1}' + @exp + ' a left join dmkh b on a.ma_kh = b.ma_kh where stt_rec = @stt_rec'
...
END";
            sql = string.Format(sql, this.MasterTable, this.PrimeTable, this.DetailTable, this.ServicesTable, this.DiscountTable, this.PaidTable, this.WarrantyTable, this.EInvoiceTable);
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@vc_id",
            ...
            if (ds != null && ds.Tables.Count >= 6)
            {
                ...
                IList<EInvoiceInfo> einvoice = ds.Tables[6].ToList<EInvoiceInfo>();
        ...
        exec fs_Calc$Discount$BHA @ma_cuahang, @ma_kh, @ngay_lap, @buy_item";
            ...
            if (ds != null && ds.Tables.Count >= 1)
            {
                ...
                {
                    DiscountDictionay.Add(item.ma_ck, item);
                }
