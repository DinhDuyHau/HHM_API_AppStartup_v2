...
namespace Voucher.IPNTran
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PNN";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c572$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m572$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i572$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d572$";
        private const string _DETAIL_PARA = "d572";

        //Chuỗi format phục vụ tạo dữ liệu tại bảng inquiry
        public string Operation { get; } = "ma_kh,ma_dvcs,ma_cuahang,ma_ca;#10$,#20$,#30$, #40$; , , , :ma_kho,ma_vt,ma_imei;#10$,#20$,#30$;d572,d572,d572";
        /// <summary>
        /// Chuỗi truy vấn khi load chứng từ
        /// </summary>
        public string LoadingQuery { get; } = "exec MokaOnline$App$Voucher$Loading '@@VOUCHER_CODE', '@@MASTER_TABLE', '@@PRIME_TABLE', 'ngay_ct', 'convert(char(6), {0}, 112)', '000000', 0, 'stt_rec', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,ngay_ct,rtrim(so_ct) as so_ct,rtrim(ma_gd) as ma_gd,rtrim(ma_cuahang) as ma_cuahang, rtrim(ma_kho) as ma_kho,rtrim(ma_khox) as ma_khox,rtrim(Dien_giai) as dien_giai,t_so_luong, t_tien_nt,t_tien,rtrim(ma_nt) as ma_nt,rtrim(ma_ct) as ma_ct,rtrim(status) as status,rtrim(user_id0) as user_id0,rtrim(user_id2) as user_id2,datetime0,datetime2,fnote3', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,ngay_ct,rtrim(so_ct) as so_ct, rtrim(ma_gd) as ma_gd, rtrim(a.ma_kho) as ma_kho,rtrim(a.ma_khox) as ma_khox, rtrim(a.Dien_giai) as dien_giai, t_so_luong, t_tien_nt,t_tien,rtrim(ma_nt) as ma_nt,rtrim(a.ma_ct) as ma_ct,rtrim(a.status) as status,rtrim(a.user_id0) as user_id0,rtrim(a.user_id2) as user_id2,a.datetime0,a.datetime2,x.statusname,y.comment,z.comment2,'''' as Hash', 'a left join dmttct x on a.status = x.status and a.ma_ct = x.ma_ct and a.ma_gd = x.loai_gd left join @@SYSDATABASE..userinfo y on a.user_id0 = y.id left join @@SYSDATABASE..userinfo z on a.user_id2 = z.id where a.ma_cuahang = ''@@SHOP_ID'' ', '@@ORDER_BY', @@ADMIN, @@USER_ID, 1, 0, '', 'ma_cuahang = ''" + Startup.Shop + "'''";
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            query += $"update {prime_table} set status = @status, ma_ca = @ma_ca, dien_giai = @dien_giai, ma_nt = @ma_nt," +
                ...
                $" ma_gd = @ma_gd, loai_ct = @loai_ct, so_ct0 = @so_ct0, so_seri0 = @so_seri0, ngay_ct0 = @ngay_ct0, user_id2 = {user_id}, datetime2 = getdate()";
        ...
        CommonObjectModel checkImeiInsert(VoucherItem vc_item)
        {
            ...
            if (vc_item.details.Any(x => x.Id == 1))
            {
                ...
                if (item_detail != null)
                {
                    ...
                    {
                        var IPDetail = item as IPDetail;
                        if (IPDetail != null && !string.IsNullOrEmpty(IPDetail.ma_imei))
                        {
                            listImei.Add(IPDetail.ma_imei.Trim());
                            ma_cuahang = IPDetail.ma_cuahang;
                        }
                    }
