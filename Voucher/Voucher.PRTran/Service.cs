...
namespace Voucher.PRTran
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PR1";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c591$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m591$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i591$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d591$";
        private const string _DETAIL_PARA = "d591";

        //Chuỗi format phục vụ tạo dữ liệu tại bảng inquiry
        public string Operation { get; } = "dept_id,loai_ct,ma_gd,ma_nt,ma_cuahang,ma_ca;#10$,#15$,#20$,#30$,#40$,#50$; , , , , , :ma_kho,ma_dc,ma_vt,ma_kh;#10$,#20$,#30$,#40$;d591,d591,d591, ";

        /// <summary>
        /// Chuỗi truy vấn khi load chứng từ
        /// </summary>
        public string LoadingQuery { get; } = "exec MokaOnline$App$Voucher$Loading '@@VOUCHER_CODE', '@@MASTER_TABLE', '@@PRIME_TABLE', 'ngay_ct', 'convert(char(6), {0}, 112)', '000000', 0, 'stt_rec', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,ngay_ct,rtrim(so_ct) as so_ct,rtrim(ma_cuahang) as ma_cuahang,rtrim(dept_id) as dept_id,rtrim(dien_giai) as dien_giai,rtrim(ma_md) as ma_md,t_tien_nt,rtrim(ma_nt) as ma_nt,rtrim(ma_ct) as ma_ct,rtrim(status) as status,rtrim(user_id0) as user_id0,rtrim(user_id2) as user_id2,datetime0,datetime2', 'rtrim(stt_rec) as stt_rec,rtrim(ma_dvcs) as ma_dvcs,ngay_ct,rtrim(so_ct) as so_ct,rtrim(a.dept_id) as dept_id,b.ten_bp,rtrim(a.dien_giai) as dien_giai,rtrim(ma_md) as ma_md,t_tien_nt,rtrim(ma_nt) as ma_nt,rtrim(a.ma_ct) as ma_ct,rtrim(a.status) as status,rtrim(a.user_id0) as user_id0,rtrim(a.user_id2) as user_id2,a.datetime0,a.datetime2,x.statusname,y.comment,z.comment2,'''' as Hash', 'a left join dmbp b on a.dept_id = b.ma_bp left join dmttct x on a.status = x.status and a.ma_ct = x.ma_ct left join @@SYSDATABASE..userinfo y on a.user_id0 = y.id left join @@SYSDATABASE..userinfo z on a.user_id2 = z.id where a.ma_cuahang = ''@@SHOP_ID'' ', '@@ORDER_BY', @@ADMIN, @@USER_ID, 1, 0, ''";
...
SELECT is_success, message FROM @check";
            ...
            if(old_voucher != null)
            {
                ...
                vc_item.ma_ca = old_voucher.ma_ca;
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            string prime_table = this.PrimeTable.Trim() + expression;
            query += "\n\n";
            query += $"update {prime_table} set dien_giai = @dien_giai, user_id2 = {user_id}, datetime2 = getdate()";
