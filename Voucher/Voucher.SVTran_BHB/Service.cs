...
namespace Voucher.SVTran_BHB
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "BHB";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c592$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m592$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i592$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d592$";
        private const string _DETAIL_PARA = "d592";
        //Bảng lưu dữ liệu chi tiết thanh toán của chứng từ
        public string PaidTable { get; } = "d592tt$";
        private const string _PAID_PARA = "d592tt";
        //Bảng lưu dữ liệu chi tiết Bảo hành của chứng từ
        public string WarrantyTable { get; } = "d592bh$";
        private const string _WARRANTY_PARA = "d592bh";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("SVTran_BHB");
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            query += "\n\n";
            //query += $"insert into {prime_table} (stt_rec, ma_ct, so_ct, ngay_ct, ma_dvcs, ma_cuahang, ma_ca, status, dien_giai, ma_kh, ma_gd, ma_nt, ty_gia, loai_ct, ma_thue, t_so_luong, t_tien_nt2, t_thue_nt, t_tt_nt, so_lo, ngay_lo, ma_nk, ngay_lct, ong_ba, ma_nx, tk, t_tien_nt, t_tien, t_thue, t_tt, ma_tt, t_tien2, t_tien_km, t_tien_km_nt, t_thue_km, t_thue_km_nt, t_km, t_km_nt, t_tc_tien2, t_tc_tien_nt2, t_tc_thue, t_tc_thue_nt, t_tc_tt, t_tc_tt_nt, t_ck, t_ck_nt, t_cp_bh, t_cp_bh_nt, t_cp_vc, t_cp_vc_nt, t_cp_khac, t_cp_khac_nt, t_cp, t_cp_nt, ma_nvbh, ck_thue_yn, ma_gia, ten_vtthue, ghi_chuthue, tk_ck, so_seri, thue_suat, tk_thue_no, tk_thue_co, ma_kh2, nam, ky, xtag, sl_in, so_dh, nhieu_thue_suat, user_id0, user_id2, datetime0, datetime2) ";
            //query += $" select @stt_rec, @ma_ct, @so_ct, @ngay_ct, @ma_dvcs, @ma_cuahang, @ma_ca, @status, @dien_giai, @ma_kh, @ma_gd, @ma_nt, @ty_gia, @loai_ct, @ma_thue, @t_so_luong, @t_tien_nt2, @t_thue_nt, @t_tt_nt, @so_lo, @ngay_lo, @ma_nk, @ngay_lct, @ong_ba, @ma_nx, @tk, @t_tien_nt, @t_tien, @t_thue, @t_tt, @ma_tt, @t_tien2, @t_tien_km, @t_tien_km_nt, @t_thue_km, @t_thue_km_nt, @t_km, @t_km_nt, @t_tc_tien2, @t_tc_tien_nt2, @t_tc_thue, @t_tc_thue_nt, @t_tc_tt, @t_tc_tt_nt, @t_ck, @t_ck_nt, @t_cp_bh, @t_cp_bh_nt, @t_cp_vc, @t_cp_vc_nt, @t_cp_khac, @t_cp_khac_nt, @t_cp, @t_cp_nt, @ma_nvbh, @ck_thue_yn, @ma_gia, @ten_vtthue, @ghi_chuthue, @tk_ck, @so_seri, @thue_suat, @tk_thue_no, @tk_thue_co, @ma_kh2, @nam, @ky, @xtag, @sl_in, @so_dh, @nhieu_thue_suat, {user_id}, {user_id}, getdate(), getdate() ";
            query += $"{insert_prime_table_query}";

            //insert các bảng chi tiết
            DetailQuery? detail_query = null;
            string detail_table = "", trans_table = "", paid_table = "", warranty_table = "";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _PAID_PARA))
            {
                ...
                //query += "\n\n";
                //query += $"insert into {paid_table} (stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_thanhtoan, ma_nvbh_i, line_nbr, tien, tien_nt, so_the_nh, ma_chuan_chi, ma_may_pos, tk_nh_nhan, so_hd_vnpay, vi_dien_tu, so_hd_tragop, tien_phi_bh, ma_dv_tragop) select stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_thanhtoan, ma_nvbh_i, line_nbr, tien, tien_nt, so_the_nh, ma_chuan_chi, ma_may_pos, tk_nh_nhan, so_hd_vnpay, vi_dien_tu, so_hd_tragop, tien_phi_bh, ma_dv_tragop from @{_PAID_PARA}";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _WARRANTY_PARA))
            {
                ...
                //query += "\n\n";
                //query += $"insert into {warranty_table} (stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_ttbh, ma_imei, hang_sx, ma_vt) select stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_ttbh, ma_imei, hang_sx, ma_vt from @{_WARRANTY_PARA}";
                query += $"{insert_warranty_query}";
            }
            if (voucherQuery.Details.Any(x => x.ParaName == _TRANS_PARA))
            {
                detail_query = voucherQuery.Details.FirstOrDefault(x => x.ParaName == _TRANS_PARA);
                trans_table = detail_query?.TableName + (detail_query.Partition_yn ? expression : "");

                query += "\n\n";
                query += detail_query?.QueryString;
                query += "\n";
                query += $"update @{_TRANS_PARA} set line_nbr = row_id$, stt_rec0 = right(row_id$ + 1000, 3), stt_rec = @stt_rec, ma_ct = @ma_ct, ngay_ct = @ngay_ct, so_ct = @so_ct where 1=1";
                query += "\n\n";
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_PARA))
            {
                ...
                query += $"insert into {detail_table} (stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,ma_vt,km_yn,ma_sp,ma_bp,so_lsx,dvt,he_so,ma_kho,ma_vi_tri,ma_lo,ma_vv,tk_vt,so_luong,gia_nt,gia,gia_nt2,gia2,tien_nt,tien,thue,thue_nt,tt,tt_nt,xstatus,xaction,tien2,tien_nt2,cp_bh,cp_bh_nt,cp_vc,cp_vc_nt,cp_khac,cp_khac_nt,cp,cp_nt,stt_rec_ct,stt_rec0ct,gia_ban0,gia_ban_nt0,gia_ban,gia_ban_nt,tl_ck,gia_ck,gia_ck_nt,ck,ck_nt,sl_dh,sl_xuat,sl_giao,stt_rec_dh,stt_rec0dh,dh_so,dh_ln,stt_rec_px,stt_rec0px,px_so,px_ln,stt_rec_gh,stt_rec0gh,stt_rec_pn,stt_rec0pn,tk_gv,tk_dt,tk_ck,tk_cpbh,px_gia_dd,ma_nvbh_i,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9,ma_thue,tk_thue_no,tk_thue_co,thue_suat,ma_imei, gia_vat)";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _PAID_PARA))
            {
                ...
                //query += $"insert into {paid_table} (stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_thanhtoan, ma_nvbh_i, line_nbr, tien, tien_nt, so_the_nh, ma_chuan_chi, ma_may_pos, tk_nh_nhan, so_hd_vnpay, vi_dien_tu, so_hd_tragop, tien_phi_bh, ma_dv_tragop)";
                //query += $"select stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_thanhtoan, ma_nvbh_i, line_nbr, tien, tien_nt, so_the_nh, ma_chuan_chi, ma_may_pos, tk_nh_nhan, so_hd_vnpay, vi_dien_tu, so_hd_tragop, tien_phi_bh, ma_dv_tragop from @{_PAID_PARA}";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _WARRANTY_PARA))
            {
                detail_query = voucherQuery.Details.FirstOrDefault(x => x.ParaName == _WARRANTY_PARA);
                warranty_table = detail_query?.TableName + (detail_query.Partition_yn ? expression : "");
                string update_warranty_query = VoucherUtils.getWarrantyQuery(new SVPaidModel(), warranty_table, _WARRANTY_PARA, 2);
                ...
                //query += $"insert into {warranty_table} (stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_ttbh, ma_imei, hang_sx, ma_vt)";
                //query += $"select stt_rec,stt_rec0, ma_ct, so_ct, ngay_ct, ma_ttbh, ma_imei, hang_sx, ma_vt from @{_WARRANTY_PARA}";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _TRANS_PARA))
            {
                ...
                //xóa dữ liệu cũ (bảng detail) và insert dữ liệu mới
                query += $"delete from {trans_table} where stt_rec = @stt_rec \n";
                query += $"insert into {trans_table} (stt_rec, ma_ct, so_ct, ngay_ct, stt_rec_hd, so_ct_hd, ngay_ct_hd, ten_kh, dia_chi, ma_loaivc, so_dh_vc, ma_van_don, tien_phi_cod, ma_nv_giao, ghi_chu_gh, ma_co, file_co, ma_cq, file_cq) ";
                query += $"select stt_rec, ma_ct, so_ct, ngay_ct, stt_rec_hd, so_ct_hd, ngay_ct_hd, ten_kh, dia_chi, ma_loaivc, so_dh_vc, ma_van_don, tien_phi_cod, ma_nv_giao, ghi_chu_gh, ma_co, file_co, ma_cq, file_cq from @{_TRANS_PARA}";
            }
            else
            {
        ...
        public CommonObjectModel Delete(string voucherId)
        {
            ...
            sql += $"delete from {this.TransTable + ngay_ct.ToString("yyyyMM")} where stt_rec = @vc_id \n";
            paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@vc_id",
        ...
        public CommonObjectModel GetById(string voucherId)
        {
            ...
            string sql = @"DECLARE @q NVARCHAR(4000), @stt_rec CHAR(13), @exp CHAR(6), @so_ct CHAR(12)
SET @stt_rec = @vc_id
IF EXISTS(SELECT 1 FROM {0} WHERE stt_rec = @stt_rec) BEGIN
	SELECT @exp = CONVERT(CHAR(6), ngay_ct, 112), @so_ct = so_ct FROM {0} WHERE stt_rec = @stt_rec
