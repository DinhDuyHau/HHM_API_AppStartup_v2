...
namespace Voucher.SVTran_BHW
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "BHW";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c561$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m561$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i561$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d561$";
        private const string _DETAIL_PARA = "d561";

        // bảng lưu dữ liệu chi tiết của dịch vụ
        public string ServicesTable { get; } = "d561dv$";
        private const string _SERVICES_PARA = "d561dv";

        // bảng lưu dữ liệu chi tiết của chiết khấu
        public string DiscountTable { get; } = "d561ck$";
        private const string _DISCOUNT_PARA = "d561ck";

        // bảng lưu dữ liệu chi tiết của thanh toán
        public string PaidTable { get; } = "d561tt$";
        private const string _PAID_PARA = "d561tt";

        // bảng lưu dữ liệu chi tiết của bảo hành
        public string WarrantyTable { get; } = "d561bh$";
        private const string _WARRANTY_PARA = "d561bh";

        // bảng dữ liệu chi tiết của vận chuyển 
        public string TransportTable { get; } = "m561ext$";
        private const string _TRANSPOSRT_PARA = "m561ext";
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("SVTran_BHW");
                    ...
                    if (item_model != null && item_detail != null)
                    {
                        switch (index_value)
                        {
                            ...
                            case 5:
                                ...
                                break;
                            case 6:
                                List<SVTransportModel>? transportModels = JsonSerializer.Deserialize<List<SVTransportModel>>((JsonElement)item_model.Data);
                                if (transportModels != null && transportModels.Count > 0)
                                {
                                    //cập nhật ngày chứng từ
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _WARRANTY_PARA))
            {
                ...
                //query += "\n\n";
                //query += $"insert into {warranty_table} (stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, ma_ttbh, ma_imei,ma_vt, hang_sx) " +
                //    $"                          select stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, ma_ttbh, ma_imei, ma_vt, hang_sx from @{_WARRANTY_PARA}";
                query += $"{insert_warranty_query}";
            }
            if (voucherQuery.Details.Any(x => x.ParaName == _TRANSPOSRT_PARA))
            {
                detail_query = voucherQuery.Details.FirstOrDefault(x => x.ParaName == _TRANSPOSRT_PARA);
                transporst_table = detail_query?.TableName + (detail_query.Partition_yn ? expression : "");

                query += "\n\n";
                query += detail_query?.QueryString;
                query += "\n";
                query += $"update @{_TRANSPOSRT_PARA} set line_nbr = row_id$, stt_rec = @stt_rec, ma_ct = @ma_ct, ngay_ct = @ngay_ct, so_ct = @so_ct where 1=1";
                query += "\n\n";
            ...
            if (!string.IsNullOrEmpty(transporst_table))
            {
                query += $"exec fs_UpdateNullToTable '{transporst_table}', '{transporst_table}', 'stt_rec = ''{stt_rec}''' \n";
            }
        ...
        public CommonObjectModel Updating(BaseModel data)
        {
            ...
            //2024-06-13: tạm bỏ qua đoạn check không cho nhập ngày quá khứ trong giai đoạn triển khai
            /*
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _WARRANTY_PARA))
            {
                ...
                //query += $"select stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, ma_ttbh, ma_imei, hang_sx, ma_vt from @{_WARRANTY_PARA}";
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _TRANSPOSRT_PARA))
            {
                ...
                //xóa dữ liệu cũ (bảng detail) và insert dữ liệu mới
                query += $"delete from {transporst_table} where stt_rec = @stt_rec \n";
                query += $"insert into {transporst_table} (stt_rec, ma_ct, ngay_ct, so_ct, ma_loaivc, so_dh_vc, ma_van_don, tien_phi_cod , ma_nv_giao, ghi_chu_gh)" +
                                                $" select stt_rec, ma_ct, ngay_ct, so_ct, ma_loaivc, so_dh_vc, ma_van_don, tien_phi_cod , ma_nv_giao, ghi_chu_gh from @{_TRANSPOSRT_PARA}";
            }
            else
