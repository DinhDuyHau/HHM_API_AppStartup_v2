...
namespace Voucher.PVTran
{
    public class Service : IVoucherService
    {
        ...
        public CommonObjectModel Updating(BaseModel data)
        {
            ...
            string sql = query_imei + @"DECLARE @check TABLE (
	is_success BIT,
	message VARCHAR(100)
)
...
IF NOT EXISTS(SELECT 1 FROM dmttct WHERE (xdefault = 1 OR xedit = 1) AND ma_ct = @vc_code AND status = @status_older) BEGIN
	...
	RETURN
END
IF EXISTS (SELECT ma_imei from imei00 where  ma_imei in (select ma_imei from @imei)) BEGIN
    UPDATE @check SET is_success = 0, message = 'imei_already_exists'
	SELECT * FROM @check
	RETURN
END
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            string prime_table = this.PrimeTable.Trim() + expression;
            query += "\n\n";
            query += $"update {prime_table} set ma_ca = @ma_ca, ma_tt = @ma_tt, ma_gd = @ma_gd, ma_kh = @ma_kh, dien_giai = @dien_giai, status = @status, user_id2 = {user_id}, datetime2 = getdate()";
