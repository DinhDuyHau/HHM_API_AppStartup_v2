...
namespace Voucher.PVTran
{
    public class Service : IVoucherService
    {
        ...
        {
            Authoriztion authoriztion = CommonService.GetAuthoriztion("PVTran");
...
SELECT is_success, message FROM @check";
        ...
        public CommonObjectModel Updated(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_PARA))
            {
                ...
                query += $"insert into {detail_table} (stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, ma_vt, dvt, he_so, ma_kho, so_luong, gia_nt, gia, gia_nt0, gia0, tien_nt, tien, ma_thue, thue_suat, thue, thue_nt, tt, tt_nt, tien0, tien_nt0, line_nbr, so_dh_i, ma_ca, ma_cuahang, ma_imei, budslive, ma_td1, ck, ck_nt, thue_ck, thue_ck_nt, s4, gc_td1, s5, sua_tien_thue_yn) select stt_rec, stt_rec0, ma_ct, ngay_ct, so_ct, ma_vt, dvt, he_so, ma_kho, so_luong, gia_nt, gia, gia_nt0, gia0, tien_nt, tien, ma_thue, thue_suat, thue, thue_nt, tt, tt_nt, tien0, tien_nt0, line_nbr, so_dh_i, ma_ca, ma_cuahang, ma_imei, budslive, ma_td1, ck, ck_nt, thue_ck, thue_ck_nt, s4, gc_td1, s5, sua_tien_thue_yn from @{_DETAIL_PARA} \r\n";
