...
namespace Voucher.DRTran
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PTN";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c542$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m542$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i542$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d542$";
        private const string _DETAIL_PARA = "d542";
        // bảng lưu dữ liệu chi tiết của thanh toán
        public string PaidTable { get; } = "d542tt$";
        private const string _PAID_PARA = "d542tt";


        public string ma_ct_goc { get; } = "PT1";
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _DETAIL_PARA))
            {
                ...
                query += "\n\n";
                query += $"insert into {detail_table} (stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,dien_giai,tk_co,tien_nt,tien,ma_kh_i,ma_vv,ma_sp,ma_bp,so_lsx,tt_qd,stt_rec_tt, so_hd_tt, ngay_hd_tt,tt,tt_nt,ty_gia_ht2,tien_ht_nt,tien_ht,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9) select stt_rec,stt_rec0,ma_ct,ngay_ct,so_ct,dien_giai,tk_co,tien_nt,tien_nt,ma_kh_i,ma_vv,ma_sp,ma_bp,so_lsx,tt_qd,stt_rec_tt, so_hd_tt, ngay_hd_tt,tt_nt,tt_nt,ty_gia_ht2,tien_ht_nt,tien_ht,line_nbr,ma_hd,ma_ku,ma_phi,so_dh_i,ma_td1,ma_td2,ma_td3,sl_td1,sl_td2,sl_td3,ngay_td1,ngay_td2,ngay_td3,gc_td1,gc_td2,gc_td3,s1,ma_ca,ma_cuahang,s4,s5,s6,s7,s8,s9 from @{_DETAIL_PARA}";
            }
            if (voucherQuery.Details.Any(x => x.ParaName == _PAID_PARA))
            {
                ...
                string insert_paid_query = VoucherUtils.getPaidQuery(new ORPaidModel(), paid_table, _PAID_PARA, 1);
