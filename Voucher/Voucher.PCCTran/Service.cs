...
using Voucher.PCCTran.Models;

namespace Voucher.PCCTran
{
    public class Service : IVoucherService
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PCC";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c554$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m554$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i554$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d554$";
        private const string _DETAIL_PARA = "d554";
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            query += "\n";
            query += $"IF @ma_thanhtoan = 'TM' set @tk = (select tk from #gdtien where 1 = 1)";
