...
namespace Voucher.OPTran
{
    ...
    {
        //Mã chứng từ
        public string VoucherCode { get; } = "PCK";

        //Bảng gốc dữ liệu không phân kỳ
        public string MasterTable { get; } = "c551$000000";

        //Bảng chính chứa dữ liệu phân kỳ
        public string PrimeTable { get; } = "m551$";

        //Bảng lưu thông tin tìm kiếm
        public string InquiryTable { get; } = "i551$";

        //Bảng lưu dữ liệu chi tiết của chứng từ
        public string DetailTable { get; } = "d551$";
        private const string _DETAIL_PARA = "d551";

        public string TaxTable { get; } = "m551ext$";
        private const string _TAX_PARA = "m551ext";
        ...
        public CommonObjectModel Inserted(object voucherItem, Query voucherQuery, int user_id)
        {
            ...
            if (voucherQuery.Details.Any(x => x.ParaName == _TAX_PARA))
            {
                ...
                query += "\n\n";
                query += $"update @{_TAX_PARA} SET tk_thue_no = (select tk_thue from #gdtien), tk_du = (select tk from #gdtien where 1 = 1) ";
                query += "\n\n";
            ...
            query += $"exec fs_UpdateNullToTable '{tax_table}', '{tax_table}', 'stt_rec = ''{stt_rec}''' \n";
...
SELECT is_success, message FROM @check";
	...
	BEGIN TRY
		if @old_partition <> @new_partition begin
			...
			select * into #tmp_ext from {old_tax_table} where stt_rec = @stt_rec
			...
			insert into {new_tax_table} select * from #tmp_ext
