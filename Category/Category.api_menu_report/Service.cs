...
namespace Category.Api_menu_report
{
    public class Service : IComponentService
    {
        ...
        public bool Inserting(Dictionary<string, object> data)
        {
            ...
            //check sql injection
            if (!data_service.IsSQLInjectionValid(entity_item.sysid) || !data_service.IsSQLInjectionValid(entity_item.controller) || !data_service.IsSQLInjectionValid(entity_item.form_id))
                throw new Exception(ApiReponseMessage.Error_InputData);

            //check tồn tại dữ liệu trong db theo khóa chính
            string sql = "select 1 from api_menu_report where sysid = @sysid and form_id = @form_id";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@sysid",
                SqlDbType = SqlDbType.Char,
                Value = entity_item.sysid
            });
            paras.Add(new SqlParameter()
            {
                ParameterName = "@form_id",
                SqlDbType = SqlDbType.Char,
                Value = entity_item.form_id
            });
