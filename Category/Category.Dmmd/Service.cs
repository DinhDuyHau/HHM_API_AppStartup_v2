...
namespace Category.Dmmd
{
    public class Service : IComponentService
    {
        ...
        public bool Inserting(Dictionary<string, object> data)
        {
            ...
            //check sql injection
            if (!data_service.IsSQLInjectionValid(entity_item.ma_md))
                throw new Exception(ApiReponseMessage.Error_InputData);

            //check tồn tại dữ liệu trong db theo khóa chính
            string sql = "select 1 from dmmd where ma_md = @ma_md, ma_ct = @ma_ct";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ma_md",
                SqlDbType = SqlDbType.Char,
                Value = entity_item.ma_md
            });
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ma_ct",
        ...
        public bool Updating(Dictionary<string, object> data)
        {
            ...
            string sql = "select * from dmmd where ma_md = @ma_md and ma_ct = @ma_ct";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ma_md",
