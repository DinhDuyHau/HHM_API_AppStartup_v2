...
namespace Employee
{
    ...
    {
        public EntityCollection<EmployeeModel> GetListEmployee(string ma_nvbh, string? order_by, int page_index = 1, int page_size = 0, string name = "")
        {
            EntityCollection<EmployeeModel> entity = null;
            string sql = "select top 1 rtrim(id) as id,name,rtrim(comment) as comment,rtrim(user_lst) as user_lst from userinfo2 where userinfo2.id in (select id from userinfo2 where user_yn = 0) and name = @name ";
            ...
            string user_lst = base.ExecSql2DataSet(sql, paras1, ConnectType.Sys).Tables[0].Rows[0]["user_lst"].ToString();
