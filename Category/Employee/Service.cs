...
namespace Employee
{
    public class Service : CoreService
    {
        ...
        public EntityCollection<EmployeeModel> GetListEmployeeApprover(string ma_nvbh, string? order_by, int page_index = 1, int page_size = 0)
        {
            EntityCollection<EmployeeModel> entity = null;
            string sql = "select top 1 rtrim(id) as id,name,rtrim(comment) as comment,rtrim(user_lst) as user_lst from userinfo2 where userinfo2.id in (select id from userinfo2 where user_yn = 0) and name = 'DUYETTANG' ";
            string user_lst = base.ExecSql2DataSet(sql, null, ConnectType.Sys).Tables[0].Rows[0]["user_lst"].ToString();

            string sql2 = @"select name as ma_nvbh, comment as ten_nvbh, ma_cuahang from userinfo where name like '%' + @ma_nvbh + '%' and name in (select ltrim(data) as name from [dbo].[MokaOnline$Function$System$Split](@user_lst, ',')) ";
            if (order_by != null && order_by != "")
            {
                sql += $"order by {order_by}";
            }
            ...
            List<EmployeeModel> raw_data = base.ExecSql2List<EmployeeModel>(sql2, paras, ConnectType.Sys);
            entity = DataUtils.PagingProcess<EmployeeModel>(raw_data, page_index, page_size);
            return entity;
        }
