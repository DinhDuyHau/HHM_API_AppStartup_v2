...
namespace Category.Dmgia_imei
{
    public class Service : IComponentService
    {
        ...
        public bool Inserting(Dictionary<string, object> data)
        {
            ...
            //check sql injection
            if (!data_service.IsSQLInjectionValid(entity_item.ma_imei))
                throw new Exception(ApiReponseMessage.Error_InputData);

            //check tồn tại dữ liệu trong db theo khóa chính
            string sql = "select 1 from dmgia_imei where ma_imei = @ma_imei and ma_nt = @ma_nt and ngay_ban = @ngay_ban";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ma_imei",
                SqlDbType = SqlDbType.Char,
                Value = entity_item.ma_imei
            });
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ma_nt",
                SqlDbType = SqlDbType.Char,
                Value = entity_item.ma_nt
            });
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ngay_ban",
                SqlDbType = SqlDbType.DateTime,
                Value = entity_item.ngay_ban
            });
