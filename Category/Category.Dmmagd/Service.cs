...
namespace Category.Dmmagd
{
    public class Service : IComponentService
    {
        ...
        public bool Inserting(Dictionary<string, object> data)
        {
            ...
            //check sql injection
            if (!data_service.IsSQLInjectionValid(entity_item.ma_gd))
                throw new Exception(ApiReponseMessage.Error_InputData);

            //check tồn tại dữ liệu trong db theo khóa chính
            string sql = "select 1 from dmmagd where ma_gd = @ma_gd and ma_ct = @ma_ct and loai_ct = @loai_ct";
            ...
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ma_ct",
                SqlDbType = SqlDbType.Char,
                Value = entity_item.ma_ct
            });
            paras.Add(new SqlParameter()
            {
                ParameterName = "@loai_ct",
                SqlDbType = SqlDbType.Char,
                Value = entity_item.loai_ct
            });
        ...
        public bool Updating(Dictionary<string, object> data)
        {
            ...
            string sql = "select * from dmmagd where ma_gd = @ma_gd and ma_ct = @ma_ct and loai_ct = @loai_ct";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter()
            {
                ParameterName = "@ma_gd",
